...
clean_rain_gauge_data <- function(excel_file) {
  ...
  # Retrieve rain station number and name from metadata
  station_row <- metadata %>%
    pull(`County of Santa Barbara`) %>%
    str_subset(pattern = "#")
  
  station_name <- str_split(station_row, pattern = " -")[[1]] %>%
    str_subset(pattern = " ") %>%
    str_remove(pattern = " ")
  
  # Add lat, long, elevation, station number, and station name to rain_data
  rain_data <- rain_data %>%
    mutate(
      lat = lat,
      long = long,
...
concat_rain_data <- function(file_list) {
  ...
  for (file in file_list){
    ...
    rain_data = rbind(rain_data, cleaned)
  }
